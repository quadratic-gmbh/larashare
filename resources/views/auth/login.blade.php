...
@endcomponent
  <div class="row justify-content-center">
    <div class="col-md-8">
      <h1 class="text-primary">{{ __('auth.login.login') }}</h1>
      <form method="POST" action="{{ route('login') }}">
        ...
        <div class="form-group row ">
          <div class="col-md-8 offset-md-4">
            <button type="submit" class="btn btn-primary">{{ __('auth.login.login') }}
            </button>
    
            @if (Route::has('password.request')) <a class="btn btn-link"
              href="{{ route('password.request') }}"> {{ __('auth.login.forgot_password') }} </a> @endif
          </div>
        </div>
        <div class="form-group row">
          <div class="col-md-8 offset-md-4">
            <a href="{{ route('register') }}">{{ __('auth.login.register') }}</a>
          </div>
        </div>
      </form>
    </div>
